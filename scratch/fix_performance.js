...
const fs = require("fs");
const files = [
    "c:\\escolaweb\\src\\main\\resources\\static\\secretary\\student-performance.html",
    "c:\\escolaweb\\src\\main\\resources\\static\\principal\\student-performance.html"
];

const navbar_html = `            <nav class="layout-navbar container-xxl navbar navbar-expand-xl navbar-detached align-items-center bg-navbar-theme shadow-sm" id="layout-navbar">
...
const tabs_html = `<div class="nav-align-top mb-4">
...
for (let file_path of files) {
    ...
    // Replace navbar
    content = content.replace(/<nav[^>]*id="layout-navbar"[\s\S]*?<\/nav>/, navbar_html);

    // Replace tabs in principal and secretary
    content = content.replace(/<div class="card">\s*<div class="card-header d-flex justify-content-between align-items-center flex-wrap">\s*<h5 class="mb-0">Resultado Consolidado<\/h5>\s*<\/div>\s*<div class="table-responsive">[\s\S]*?<\/table>\s*<\/div>\s*<\/div>/, tabs_html);
    content = content.replace(/<div class="nav-align-top mb-4">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/, tabs_html);

    // Strip modals
    content = content.replace(/<!-- Modal para Editar Nota -->[\s\S]*?<!-- Modal para Editar Falta -->[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/, "");
    content = content.replace(/<!-- Modal para Editar Nota -->[\s\S]*?<!-- Modal para Editar Falta -->[\s\S]*?<\/div>\s*<\/div>/, "");

    // Inject JS
    if (!content.includes("const tbodyNotas =")) {
        content = content.replace("tbody.innerHTML = html;", "tbody.innerHTML = html;" + js_addition);
    }

    fs.writeFileSync(file_path, content, "utf-8");
}
console.log("Fixed HTML using NodeJS");
